...
var (
	...
	cacheProblemTagIdPrefix             = "cache:problemTag:id:"
	cacheProblemTagProblemIdTagIdPrefix = "cache:problemTag:problemId:tagId:"
)

type (
	problemTagModel interface {
		Insert(ctx context.Context, data *ProblemTag) (sql.Result, error)
		FindOne(ctx context.Context, id uint64) (*ProblemTag, error)
		FindOneByProblemIdTagId(ctx context.Context, problemId uint64, tagId uint64) (*ProblemTag, error)
...
func (m *defaultProblemTagModel) Delete(ctx context.Context, id uint64) error {
	data, err := m.FindOne(ctx, id)
	if err != nil {
		return err
	}

	problemTagIdKey := fmt.Sprintf("%s%v", cacheProblemTagIdPrefix, id)
	problemTagProblemIdTagIdKey := fmt.Sprintf("%s%v:%v", cacheProblemTagProblemIdTagIdPrefix, data.ProblemId, data.TagId)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, problemTagIdKey, problemTagProblemIdTagIdKey)
	return err
}

func (m *defaultProblemTagModel) FindOne(ctx context.Context, id uint64) (*ProblemTag, error) {
	problemTagIdKey := fmt.Sprintf("%s%v", cacheProblemTagIdPrefix, id)
	var resp ProblemTag
	err := m.QueryRowCtx(ctx, &resp, problemTagIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultProblemTagModel) FindOneByProblemIdTagId(ctx context.Context, problemId uint64, tagId uint64) (*ProblemTag, error) {
	problemTagProblemIdTagIdKey := fmt.Sprintf("%s%v:%v", cacheProblemTagProblemIdTagIdPrefix, problemId, tagId)
	var resp ProblemTag
	err := m.QueryRowIndexCtx(ctx, &resp, problemTagProblemIdTagIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultProblemTagModel) Insert(ctx context.Context, data *ProblemTag) (sql.Result, error) {
	problemTagIdKey := fmt.Sprintf("%s%v", cacheProblemTagIdPrefix, data.Id)
	problemTagProblemIdTagIdKey := fmt.Sprintf("%s%v:%v", cacheProblemTagProblemIdTagIdPrefix, data.ProblemId, data.TagId)
...
func (m *defaultProblemTagModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheProblemTagIdPrefix, primary)
}
