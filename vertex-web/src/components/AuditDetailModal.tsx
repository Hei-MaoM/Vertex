...
export const AuditDetailModal = ({ problemId, onClose, onSuccess }: Props) => {
    ...
    useEffect(() => {
        ...
        const fetchDetail = async () => {
            ...
            } catch (err: any) { setError("获取失败"); } finally { setLoading(false); }
        };
        fetchDetail();
    }, [problemId]);
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl w-full max-w-4xl h-[90vh] shadow-2xl relative flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border-4 border-orange-100">
                ...
                <div className="flex-1 overflow-y-auto p-8 pb-24 bg-gray-50">
                    {loading ? <Loader2 className="animate-spin mx-auto text-orange-500" /> : error ? <div className="text-center text-red-500">{error}</div> : detail && (
                        <div className="space-y-6">
                            <div className="bg-white p-6 rounded-xl shadow-sm">
                                <h1 className="text-2xl font-bold mb-2">{detail.title}</h1>
                                <div className="text-sm text-gray-500">来源: {detail.source} | 原题: <a href={detail.problem_url} target="_blank" className="text-blue-500 underline">{detail.problem_url}</a></div>
                            </div>
                            <div className="bg-white p-6 rounded-xl shadow-sm">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm">
                                ...
                                <pre className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto text-sm">{detail.solution}</pre>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* 底部固定操作栏 */}
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 flex justify-end gap-4 shadow-lg">
                    <button onClick={() => handleAudit(2)} className="px-6 py-2 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 font-bold flex gap-2"><XIcon /> 拒绝</button>
                    <button onClick={() => handleAudit(1)} className="px-6 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 font-bold flex gap-2 shadow-md"><Check /> 通过发布</button>
                </div>
            </div>
        </div>
    );
